...
:::note
The delegated nameservers do not have to belong to a different DNS provider. A delegation to Cloudflare nameservers in another account or zone — for example, a [subdomain setup](/dns/zone-setups/subdomain-setup/) — still shadows records in the parent zone exactly the same way.
:::
