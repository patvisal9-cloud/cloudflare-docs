...
Note: Wrangler must be version 4.69.0 or above. [Per-entrypoint caching](#per-entrypoint-caching) and [cross-version caching](#cross-version-caching) require Wrangler 4.107.0 or above.
...
:::caution[Prefer disabling cache over returning `no-store` on a gateway]
