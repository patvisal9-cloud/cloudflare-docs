...
- **`206 Partial Content`** is not cached. Workers Caching does not split responses into byte-range slices, so a Worker that responds to a `Range` request with `206` will see `MISS` on every request. Return a full `200` response and let the cache handle range requests on subsequent hits.
