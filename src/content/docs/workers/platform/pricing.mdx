...
By default, users have access to the Workers Free plan. The Workers Free plan includes limited usage of Workers, Pages Functions, Workers KV and Hyperdrive. Read more about the [Free plan limits](/workers/platform/limits/#account-plan-limits).
...
<sup>2</sup> WebSocket connections made to a Worker are charged as a request,
representing the initial `Upgrade` connection made to establish the WebSocket.
WebSocket messages routed through a Worker do not count as requests.
...
invoke the Worker, and requests that are normally free — [static
assets](/workers/static-assets/billing-and-limitations/) and [worker-to-worker
invocations](/workers/platform/pricing/#service-bindings) — are also billed at
the standard request rate. CPU time is only billed when the Worker runs (on a
